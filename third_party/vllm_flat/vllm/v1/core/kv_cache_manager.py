...
class KVCacheManager:
    ...
    def get_computed_blocks(self,
        ...
        # ---- Mimir patch (Phase D): 分支 CoW 复用记账 ----
        # 当请求复用了「属另一 agent 任务/分支」的缓存块时，记为一次 CoW 复用
        # （v1 prefix caching 本身就复用同前缀块；这里统计跨分支复用次数，
        #  让 Mimir 外部 BranchTree 的 CoW 节省成为引擎实测的投影）。
        try:
            req_branch = getattr(request, "mimir_task_id", None)
            bp = self.block_pool
            cow = 0
            for blk_group in computed_blocks:
                for blk in blk_group:
                    bid = blk.block_id
                    owner = bp.mimir_block_task.get(bid, None)
                    if owner is not None and owner != req_branch:
                        cow += 1
            if cow:
                bp.mimir_cow_reuses = getattr(bp, "mimir_cow_reuses", 0) + cow
        except Exception:
            pass
        # ---- Mimir patch end ----
