...
class Scheduler(SchedulerInterface):
    ...
    # ---- Mimir in-tree patch: 块级统计导出（Phase B）------------------------- #
    # 目的：让 Mimir（父进程）能读到 block_pool 的块级使用，而无需遍历对象图。
    # 与 vLLM 原生 make_stats 的区别：后者只有 kv_cache_usage(float)，这里给出
    # used_blocks/total_blocks/utilization，是 KV 优化（压缩/外置/CoW/淘汰）最敏感的口径。
    def get_mimir_stats(self) -> dict:
        ...
        try:
            ...
            stats = {
                "used_blocks": used,
                "total_blocks": total,
                "utilization": bp.get_usage() if hasattr(bp, "get_usage") else
