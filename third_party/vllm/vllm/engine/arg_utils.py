...
class EngineArgs:
    ...
    def _is_v1_supported_oracle(self, model_config: ModelConfig) -> bool:
        ...
        if self.kv_cache_dtype != "auto":
            ...
            if not supported:
                # ---- Mimir patch (Phase F): fp8 KV 在不支持硬件/后端上优雅降级到 bf16 ----
                # 原行为：raise NotImplementedError（任务直接失败）。Mimir 改为：warn + 回退 auto，
                # 让 agent 任务继续跑（保成功率），仅放弃 fp8 的显存收益。
                from vllm.logger import init_logger as _mimir_logger
                _lg = _mimir_logger("vllm.engine.arg_utils")
                _lg.warning(
                    "Mimir Phase F fallback: --kv-cache-dtype=%s not supported on "
                    "this device/backend under V1; falling back to bf16 (auto) KV. "
                    "Task continues; fp8 capacity gain unavailable here.",
                    self.kv_cache_dtype)
